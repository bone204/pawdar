...
Return the result as a raw JSON object only (no markdown code blocks):
{
  ...
  async moderateComment(
    ...
    if (!this.apiKey) {
      this.logger.warn(
        'GEMINI_API_KEY is not defined, skipping comment moderation (defaulting to approved).',
      );
      return { isApproved: true, label: 'clean' };
    }
    ...
    try {
      const languageText = lang === 'en' ? 'English' : 'Vietnamese';
      const prompt = `You are an AI content moderation assistant for the pet community app Pawdar.
Analyze the following post comment to check if it contains severe profanity, swear words, harassment, hate speech, explicit adult content, or malicious spam.

Comment: "${content}"

CRITICAL INSTRUCTIONS:
- Do NOT be overly strict. Common pet terms, harmless slang, and standard descriptions are completely CLEAN and MUST be approved. Only reject genuinely offensive, toxic, or vulgar language.
- The feedback "reason" MUST be written in ${languageText}.
...
Return the result as a raw JSON object only (no markdown code blocks):
{
  ...
  "reason": "Detailed rejection reason in ${languageText} explaining which specific words or parts violated the policy (set to null if isApproved is true)"
      ...
      if (!response.ok) {
        const errorText = await response.text();
        this.logger.error(
          `Gemini API Error for comment: ${response.status} - ${errorText}`,
        );
        return { isApproved: true, label: 'clean' };
      }
      ...
      this.logger.error('Failed to moderate comment via Gemini', error);
      return { isApproved: true, label: 'clean' };
    }
  }
}
